    ...
    if (mode === 'yield') {
      ...
      return done(new ol.layer.Vector({
        ...
        style: f => new ol.style.Style({
          fill: new ol.style.Fill({ color: (typeof f.get(fld) === 'number') ? ramp[getBreakClass(f.get(fld), breaks)] : '#ccc' }),
    ...
    if (mode === 'dotdensity') {
      ...
      const bgLayer = new ol.layer.Vector({
        ...
        style: f => {
          ...
          const color = (typeof y === 'number') ? yieldColors[getBreakClass(y, yieldBreaks)] : '#ccc';
          return new ol.style.Style({
            fill: new ol.style.Fill({ color }),
            stroke: new ol.style.Stroke({ color: '#555', width: 0.5 })
          });
...
box.innerHTML += '<div style="margin-top:6px;font-size:11px;">No data: <span style="display:inline-block;width:20px;height:20px;background:#ccc;border:1px solid #aaa;"></span></div>';
  ...
  if (mode === 'bivariate') {
    ...
    fetch(geojsonUrl).then(r => r.json()).then(data => {
      ...
      xLabelsDiv.style.display = 'grid';
      xLabelsDiv.style.gridTemplateColumns = 'auto repeat(5, 24px)';
      xLabelsDiv.style.marginTop = '4px';
      xLabelsDiv.style.fontSize = '10px';

      xLabelsDiv.appendChild(document.createElement('div'));
      for (let x = 0; x < 5; x++) {
        const lbl = document.createElement('div');
        lbl.style.textAlign = 'center';
        lbl.innerHTML = `${Math.round(abreaks[x])}<br>${Math.round(abreaks[x + 1])}`;
        xLabelsDiv.appendChild(lbl);
      }

      box.appendChild(xLabelsDiv);

      const xTitle = document.createElement('div');
      xTitle.innerHTML = '<strong style="font-size:12px;">Area Harvested (ha)</strong>';
      xTitle.style.textAlign = 'center';
      xTitle.style.marginTop = '6px';
      box.appendChild(xTitle);
